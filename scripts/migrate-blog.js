...
async function migrate() {
    const migrationFile = path.join(__dirname, '../supabase/migrations/20251201_create_blog.sql');
    const query = fs.readFileSync(migrationFile, 'utf8');

    console.log('Running migration for blog articles...');
    try {
        await sql.unsafe(query);
        console.log('✅ Migration successful: articles table created.');
    } catch (e) {
