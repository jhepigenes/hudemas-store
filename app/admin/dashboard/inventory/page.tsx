    ...
    const handleAddProduct = async () => {
        try {
            ...
            const { error } = await supabase.from('marketplace_listings').insert({
                ...
                user_id: user.id,
                status: 'active', // Admin adds are active by default
                currency: 'RON' // Default for now
            });

            if (error) throw error;

            // Refresh products (mock refresh for now, ideally re-fetch)
            alert("Product added successfully!");
            setIsProductModalOpen(false);
            setNewProduct({ title: '', price: '', stock: 1, product_type: 'kit', description: '' });
        } catch (error) {
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="w-full max-w-md bg-white dark:bg-stone-900 rounded-xl shadow-xl p-6 border border-stone-200 dark:border-stone-800">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-lg font-serif font-medium text-stone-900 dark:text-white">Add New Product</h3>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="mt-1 block w-full rounded-md border-stone-300 shadow-sm focus:border-stone-500 focus:ring-stone-500 dark:bg-stone-800 dark:border-stone-700 h-20 resize-none"
                                />
                            </div>
                            <button
                                onClick={handleAddProduct}
                                className="w-full mt-4 rounded-md bg-stone-900 px-4 py-2 text-sm font-medium text-white hover:bg-stone-800 dark:bg-white dark:text-stone-900"
                            >
                                Add Product
                            </button>
                        </div>
